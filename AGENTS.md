...
- **Cardinal Adapter Rule ("Only Mock Types You Own")**: Never mock third-party libraries, SDKs, ORMs, drivers, or external clients. Encapsulate external dependencies behind owned boundary interfaces; mock only these interfaces in unit tests.
...
- **Fail-Fast Runtime Config**: Validate environment variables at application startup using a stack-native schema validator. Abort boot on malformed, missing, or insecure values (e.g., reject weak secrets and wildcard CORS in production).
- **Standard Error Envelope**: Standardize all API error responses: `{ "error": string, "code": string, "requestId": string, "details"?: unknown }`. Mask 500 errors and stack traces in production.
...
## 7. Frontend Architecture, Design Tokens, Theming & A11y

- **Foundation Defined at Start**: Lock in UI framework, CSS strategy (utility-first, CSS Modules, or modern zero-runtime CSS), typography scale, and layout primitives at project inception.
- **Semantic Design Tokens**:
  - Define design tokens via CSS custom properties (`--bg-canvas`, `--bg-surface`, `--text-main`, `--text-muted`, `--border-base`, `--accent-primary`, etc.).
...
- **Theme Engine (Light/Dark/System)**:
  - Drive themes via root attributes (`data-theme="dark"` or `class="dark"`).
  - Support user preference persistence with seamless fallback to `prefers-color-scheme`.
...
- **Accessible Interaction Primitives (A11y)**:
  ...
  - Forbid native browser dialogs (`window.confirm`, `window.alert`) and inline layout-shifting forms; use accessible modals/dialogs.
